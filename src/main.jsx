...
const shouldRedactLocalStorageKey = (key = "") => {
...
const sanitizeLocalStorageValue = (key, value) => {
...
const DEBUG_MODE_STORAGE_KEY = "openwhisprDebugEnabled";
...
const getLocalStorageSnapshot = () => {
  ...
  try {
    ...
    snapshot.__error = e?.message || String(e);
  }
  return snapshot;
};

async function bootstrapDebugTelemetry() {
  try {
    if (typeof window === "undefined" || !window.electronAPI?.getDebugState) {
      return;
    }
    if (window.__openwhisprDebugTelemetryBootstrapped) {
      return;
    }
    window.__openwhisprDebugTelemetryBootstrapped = true;
