...
export const createRecordingOperationQueue = () => {
...
export const createStopRecordingHandler = (deps) => {
  ...
  return async (payload = {}) => {
    ...
    const normalizedPayload = normalizeTriggerPayload({
      ...payload,
    ...
    if (hasSessionMismatch) {
      logger.debug(
        "Ignoring stale stop payload session identity",
        {
          requestedSessionId,
          activeSessionId,
        },
    ...
    if (!activeSessionRef.current) {
      ...
      // Stop events may be generated by a separate tap callback. Preserve the active identity and
      // merge only stop timing/details, so stale payload IDs cannot split one dictation into two.
      activeSessionRef.current = {
        ...activeSessionRef.current,
        ...(normalizedPayload.startedAt ? { startedAt: normalizedPayload.startedAt } : {}),
        ...(normalizedPayload.releasedAt ? { releasedAt: normalizedPayload.releasedAt } : {}),
        ...(normalizedPayload.stopReason ? { stopReason: normalizedPayload.stopReason } : {}),
        ...(normalizedPayload.stopSource ? { stopSource: normalizedPayload.stopSource } : {}),
      };
    }

    const session = activeSessionRef.current;
    const stopSource =
      normalizedPayload?.stopSource || (normalizedPayload?.releasedAt ? "released" : "manual");
    const stopReason =
