...
- Native Android code lives under `android/app/src/main`. `RecorderService` owns the microphone foreground-service lifecycle, `PendingRecordingStore` owns app-private retry files, `InboxTreeStore` owns the persisted Storage Access Framework grant, `SafInboxPublisher` owns ordered protocol publication, and `EchoDraftWidgetProvider` owns widget actions. Keep these responsibilities separate and avoid adding a mobile networking or transcription layer.
