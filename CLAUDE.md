...
- Native Android code lives under `android/app/src/main`. `RecorderService` owns the microphone foreground-service lifecycle, `PendingRecordingStore` owns app-private memo retry files, `InboxTreeStore` owns the persisted Storage Access Framework grant, `SafInboxPublisher` owns ordered memo publication, `MobileDiagnosticStore` owns the bounded private failure snapshot, `SafDiagnosticSink` owns its best-effort shared-folder copy, `MobileDiagnosticReporter` serializes private writes and isolates potentially stalled provider work from component callbacks, and `EchoDraftWidgetProvider` owns widget actions. Keep these responsibilities separate and avoid adding a mobile networking or transcription layer.
...
- Android publishes audio first and read-verifies each document before treating publication as complete. It closes the complete audio document before publishing the ready manifest; where the selected provider advertises rename support, it writes the manifest under a temporary name and renames it last. Otherwise it creates and closes the final ready document only after the audio write succeeds. A no-backup publication journal records each app-created final document URI before writing, so retry may remove a partial exact-name document only when ownership is durable and the provider advertises deletion; foreign collisions and incompatible providers fail without retiring the private phone copy.
