...
- **android.ps1**: Tests, lints, builds, and optionally sideloads the private Android companion using Android Studio's bundled toolchain
...
- The private Android companion publishes through Microsoft Graph into its delegated OneDrive app folder, `Apps/EchoDraft Mobile Inbox`; the desktop watches that folder's ordinary local OneDrive copy. The desktop exposes no HTTP listener, changes no firewall rule, and stores no phone/device identifier.
- Native Android code lives under `android/app/src/main`. `RecorderService` owns the microphone foreground-service lifecycle, `PendingRecordingStore` owns app-private memo retry files, `OneDriveSession` owns single-account MSAL interaction, `MicrosoftGraphDriveApi` owns bounded Graph HTTP, `OneDriveRemoteFiles` owns conditional remote-file integrity, `GraphInboxPublisher` owns protocol ordering, `MobileDiagnosticStore` owns and validates the bounded private failure snapshot, `GraphDiagnosticSink` owns its best-effort app-folder copy, `MobileDiagnosticReporter` isolates diagnostic work from component callbacks, and `EchoDraftWidgetProvider` owns local-only widget actions. Do not add a custom backend, mobile transcription, broad OneDrive scope, multi-account product surface, scheduler, or database without an explicit requirement.
- Private build values come only from ignored `android/local.properties`. The app is single-tenant and requests exactly delegated `Files.ReadWrite.AppFolder`. MSAL owns token storage; EchoDraft must not store, return, or log tokens, account names, provider payloads, or raw MSAL errors. The manifest callback path and URL-encoded MSAL JSON redirect must derive from the same raw signature hash.
- Android writes and verifies audio before creating the ready manifest. New path uploads set Graph conflict behavior to `fail`; intentional diagnostic updates address the known item ID and use its eTag. After manifest commit, both final names, item identities/eTags, sizes, and bytes/hashes are checked again. A mismatched ready marker is conditionally removed, collisions fail closed, and `RecorderService` deletes the private memo only after `GraphInboxPublisher.publish` completes successfully.
...
- `MobileInboxManager` owns desktop folder configuration, five-second polling while the dictation renderer is ready,
  retry/quarantine state, idempotency checks, and orchestration. `MobileInboxFileStore` owns canonical-root snapshots, bounded
  stable-handle reads, identity/hash revalidation, atomic private claims, quarantine, and exact-pair deletion.
