...
- **Unambiguous Dictation Sounds**: Uses a bright rising cue for confirmed recording start, a lower falling sweep for confirmed stop/processing, a separate chime-and-pop for successful text delivery, and dedicated cancellation and error sounds.
- **Recording Timer**: Shows a compact, click-through `REC` timer while the microphone is live, without taking focus from the active application.
- **Sound & Feedback Settings**: Adds sound enablement, volume, recording-timer visibility, and individual cue previews under General settings.

### Fixed

- **Truthful Completion Feedback**: Plays the completion cue only after text was inserted or copied successfully; failed insertion and clipboard delivery now use the error cue.

## [1.4.7] - 2026-07-13

### Added

- **Control Panel Microphone Picker**: Adds a prominent microphone selector beside the dictation shortcuts, with automatic built-in, Windows default, explicit-device, refresh, and unplugged-device states.
- **Private Device-Name Reveal**: Lists available inputs without requesting capture permission and offers a deliberate action when Windows requires microphone access to reveal device names.

### Fixed

- **Microphone Selection Consistency**: Keeps the control panel and Preferences synchronized, preserves Windows default as an intentional choice, and filters duplicate or non-targetable browser device aliases.
- **USB Device Refresh**: Prevents older overlapping device scans from replacing newer plug/unplug results and validates cached automatic microphone IDs before recording.

## [1.4.6] - 2026-07-12

### Added

- **Cleanup Fidelity Guard**: Detects suspicious compression, changed negation, lost questions or critical literals, leaked wrappers, and assistant-style execution; retries once in strict-preservation mode and otherwise keeps the original transcript.
- **Dictation Shortcuts Card**: Shows both active shortcuts, activation behavior, cleanup status, selected model, and direct links to the relevant settings.
- **Real-Audio Regression Harness**: Opt-in end-to-end evaluation for authorized recordings, cleanup safety cases, silence handling, and independent preservation review.

### Fixed

- **Truncated OpenAI Transcriptions**: Streaming responses must now reach an explicit completion marker; interrupted streams are retried once through the complete non-streaming path.
- **Streaming Finalization**: AssemblyAI text is accepted only after the service sends its explicit termination event; missing, closed, failed, and timed-out sessions now fail closed without exposing partial text for insertion.
- **AI Cleanup Availability**: Replaced the unavailable legacy default with the current GPT-5.6 Terra cleanup profile and made provider/model availability and fallback outcomes truthful in history and notifications.
- **Over-Summarised Cleanup**: Strengthened the editing contract to preserve every substantive clause, qualifier, grammatical subject, question, condition, and polarity while treating dictated requests as untrusted text rather than instructions.
- **Windows Hotkey Reliability**: Suppressed key-repeat toggles, kept stop events on the active recording session, restarted failed native listeners, and recovered shortcuts after resume or workstation unlock.
- **Sensitive Diagnostics**: Removed API-key fragments and transcript previews from provider and pipeline logs while retaining useful lengths, timings, and status metadata.
- **Push-to-Talk Safety**: Force-stops active sessions if a native listener, suspend/recovery cycle, or controller shutdown can lose key-up; also adds a bounded recording timeout and global toggle fallback.
- **Delivery Truthfulness**: Separates transcription success from insertion/clipboard delivery, guarantees a clipboard fallback where possible, labels delivery issues accurately in history and exports, and keeps a failed dictation recoverable from the tray without persisting extra transcript content.
- **Release Isolation**: Forces packaged E2E runs into unique profiles, verifies the production database remains byte-identical, pins the reviewed native helper by source and binary SHA-256, and keeps foreground/typing automation behind an explicit idle-desktop opt-in.
- **OAuth Log Privacy**: Removes query strings and fragments from renderer telemetry so session verifiers and callback values cannot enter debug logs.

### Changed

- **History UX**: Clarified history controls, search, cleanup/fallback badges, and accessible labels while making the main list use the available window height more effectively.
- **Settings UX**: Added a dedicated Shortcuts destination with direct navigation, focused controls, visible keyboard focus, and current-page semantics.
- **OpenAI Cleanup Requests**: Added adaptive output budgets and timeouts, explicit no-reasoning cleanup settings, and strict handling for incomplete or truncated model responses.
...
- **Menu Labelling**: macOS application menu items now display the correct EchoDraft casing everywhere in the shell UI.
