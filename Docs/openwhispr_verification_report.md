...
|---|---|---|---|---|---|
| A | Dual hotkeys register (Insert + Clipboard) | `src/helpers/windowManager.js` (`updateHotkey`, `registerClipboardHotkeyInternal`), `src/helpers/ipcHandlers.js` (`e2e-get-hotkey-status`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Hotkeys registered (insert+clipboard)” | PASS | Gate auto-selects non-conflicting pair (e.g., `F8` + `F9`). |
| A | Insert mode inserts into target (foreground stable) | `src/hooks/useAudioRecording.js` (`handleTranscriptionComplete` Insert path), `src/helpers/clipboard.js` (`pasteText`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Insert mode writes into GatePad (foreground stable)” | PASS | Gate uses GatePad by default to avoid touching user Notepad sessions. |
| A | Clipboard mode never auto-inserts; copies to clipboard | `src/hooks/useAudioRecording.js` (Clipboard path writes clipboard; no paste), `src/helpers/windowManager.js` (clipboard route payload) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Clipboard mode does not insert”, **PASS** “Clipboard mode copies to clipboard” | PASS | Gate verifies target text unchanged + clipboard contains result. |
| A | Push-to-talk mode works for both hotkeys (Windows native listener wiring) | `main.js` (Windows Push-to-Talk + `activation-mode-changed` + `refreshWindowsKeyListeners`), `src/helpers/windowsKeyManager.js` (`start(key, hotkeyId)`), `resources/windows-key-listener.c` | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Push-to-talk mode uses native listener (both routes)” | PASS | Listener stop/restart is now treated as normal (no spurious “unavailable” toast). |
| A | “While processing, switch apps” behavior matches mode | `src/helpers/clipboard.js` (`activateInsertionTarget` + failure path), `src/hooks/useAudioRecording.js` (records `pasteSucceeded`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Switch focus away before insert…”, **PASS** “Target lock inserts… OR falls back to clipboard” | PASS | Gate uses a decoy foreground window to force activation logic. |
| B | Status/progress bar always visible | `src/components/ui/DictationStatusBar.jsx`, `src/App.jsx` | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Status bar present” | PASS | |
| B | Stage labels update (Listening/Transcribing) | `src/hooks/useAudioRecording.js` (`STAGE_META`, `updateStage`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Stage label updates (Listening)”, “(Transcribing)” | PASS | Gate asserts UI label changes via E2E helper. |
| C | History workspace renders | `src/components/ControlPanel.tsx`, `src/stores/transcriptionStore.ts` | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “History renders items” | PASS | |
| C | History search works | `src/components/ControlPanel.tsx` (`filteredHistory`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “History search filters results” | PASS | |
| C | History retains text even when paste fails | `src/helpers/audioManager.js` (`safePaste`), `src/hooks/useAudioRecording.js` (saves transcription with `pasteSucceeded`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Insert failure leaves text in clipboard”, **PASS** “History retains text after insert failure” | PASS | Insert failure is forced with an invalid HWND to prove safe fallback. |
| D | Per-dictation diagnostics persisted (mode/status/provider/timings) | `src/helpers/database.js` (`meta_json`, `hydrateTranscriptionRow`, `patchTranscriptionMeta`), `src/hooks/useAudioRecording.js` (writes meta + timings) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** export JSON/CSV + **PASS** “Export JSON includes diagnostics columns”, “Export CSV includes diagnostics columns” | PASS | Export includes `outputMode`, `status`, `provider`, `pasteSucceeded`, `recordMs/transcribeMs/pasteMs/totalMs` etc. |
| D | Export works (CSV + JSON) | `src/helpers/ipcHandlers.js` (`e2e-export-transcriptions`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “E2E export transcriptions (JSON/CSV)” | PASS | |
| D | Diagnostics do not include secrets | `src/helpers/environment.js` (keys stored separately), `src/helpers/ipcHandlers.js` (export flattens meta; no keys) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Export JSON includes diagnostics columns” (`secretLike=false` heuristic) | PASS | Heuristic is limited; code-path review shows API keys are not exported. |
| E | Batch dictionary paste-import supports dedupe/preview | `src/components/SettingsPage.tsx` (batch dictionary UI), `src/helpers/ipcHandlers.js` (dictionary handlers) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Dictionary preview shows dedupe counts” | PASS | |
| E | Merge applies to DB | `src/helpers/database.js` (`setDictionary`), `src/helpers/ipcHandlers.js` (`db-set-dictionary`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Dictionary merge writes to DB” | PASS | |
| E | Export/import round-trip works | `src/helpers/ipcHandlers.js` (`e2e-export-dictionary`, `e2e-import-dictionary`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “E2E export dictionary (TXT)”, “E2E import dictionary (TXT)” | PASS | |
| F | Capture insertion target at dictation start | `src/helpers/clipboard.js` (`captureInsertionTarget`), `src/hooks/useAudioRecording.js` (captures at start) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Capture insertion target … (foreground)” | PASS | Gate asserts captured HWND equals expected target HWND. |
| F | Best-effort activation of original target on completion | `src/helpers/clipboard.js` (`activateInsertionTarget`, error handling) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Target lock inserts… OR falls back to clipboard” | PASS | |
| F | Failure path is safe (clipboard + history) | `src/helpers/audioManager.js` (`safePaste`), `src/hooks/useAudioRecording.js` (saves regardless; `pasteSucceeded`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Insert failure leaves text in clipboard”, “History retains text after insert failure” | PASS | |
| F | Elevated target case handled gracefully | `src/helpers/clipboard.js` (errors surface via `safePaste`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Insert failure leaves text in clipboard” + “History retains text after insert failure” | PASS | Automated gate simulates a hard failure (invalid HWND). Manual check for UAC/elevated targets is still recommended. |
| G | Clipboard format preservation (image) across insert | `src/helpers/clipboard.js` (`snapshotClipboard`, `restoreClipboardSnapshot`) | Gate run `2026-02-11T01-54-57-866Z`: **PASS** “Clipboard image preserved after insert” | PASS | Gate uses a deterministic clipboard test image and verifies pixel-hash equality after restore delay. |
...
7) **Cloud/BYOK transcription returned the dictionary prompt instead of speech**  
   - Severity: High (core functionality; wrong output copied to clipboard + persisted to history)  
   - Repro (observed): enable custom dictionary, use OpenAI STT (e.g., `gpt-4o-transcribe`), dictate a short phrase; output becomes the dictionary term list.  
   - Root cause (likely): STT can echo the `prompt` (dictionary hint) when audio is silent/unusable or the model misbehaves; our pipeline accepted that as the transcript and the cleanup model reformatted it into a bullet list.  
   - Fix (commit): `35dea66` — detect likely “prompt echo” and automatically retry once without the dictionary prompt; if still echoed, hard-fail with a clear error. Gate now includes a heuristic regression check.
...
- Packaged runtime gate PASS on Windows (`runId 2026-02-11T01-54-57-866Z`)
