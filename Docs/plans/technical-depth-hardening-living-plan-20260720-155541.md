...
- [ ] Follow-up publish/install closeout. Current user instructions explicitly authorize the feature push, fast-forward merge and push to `origin/main`, exact latest-artifact reinstall, and Explorer-owned relaunch. Next action: inspect and commit the frozen candidate, push and verify the feature branch, fast-forward and verify `origin/main`, rebuild from that exact main SHA, install the exact hash, relaunch through Explorer, verify the installed version/processes, append evidence, and leave a clean main worktree.
